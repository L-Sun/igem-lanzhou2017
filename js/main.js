...
/* General */
    ...
    $(tiemline_item).click(function(e) {
        e.preventDefault();
        $(this).next('.container').slideToggle(400);
        $(this).toggleClass('container-opened');
    });
    $('.read_more').click(function(event) {
        event.preventDefault();
        $(this).parent().slideUp(400);
        $(this).parents('.overview').next('.detail').slideDown('400');
    });
    ...
    function callback(){
        ...
        if($('#page__drawer-mobile-container').css('display') == 'none') {
            if ($('.page__article:visible').offset().top - $(this).scrollTop() < 80 + $('.mdc-toolbar').height() + $('.mdc-toolbar').position().top) {
                $('.page__drawer').css({
                    'position': 'fixed',
                    'top': 122 + $('.mdc-toolbar').height() + $('.mdc-toolbar').position().top +  'px'
                });
                $('.page__article').css({
    ...
    function page_drawer() {
        if ($('#temp-page-list-desktop').length > 0) {
            ...
            $('.page__article:visible').children('h1').map(function(index) {
                let id = 'page__article__headline-' + index + '-' + Math.floor(Math.random()*10);
                $(this).attr('id', id);
